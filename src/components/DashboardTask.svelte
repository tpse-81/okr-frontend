...
<div class="card card-bordered bg-white-100 h-auto lg:h-125">
	<!-- Header -->
	<div
		class="p-3 lg:p-4 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-3 lg:gap-0"
	>
		<div>
			<h2 class="text-base lg:text-lg font-semibold">
				{$_("dashboard.availableTasks")}
			</h2>
		</div>

		<div
			class="flex flex-col lg:flex-row gap-2 items-start lg:items-center w-full lg:w-auto"
		>
			{#if selectedStatuses.length}
				<div class="flex gap-1 sm:gap-2 overflow-x-auto">
					{#each selectedStatuses as s}
						<span
							class="text-xs sm:text-sm badge badge-ghost flex-shrink-0 flex items-center gap-1 whitespace-nowrap"
						>
							{$_(taskStateMap[s].label)}
							<button
								class="ml-1 btn btn-ghost btn-xs p-0"
			...
			<div class="dropdown dropdown-bottom">
				<div
					tabindex="0"
					role="button"
					class="btn btn-xs sm:btn-sm btn-outline flex items-center gap-2"
				>
					{$_("dashboard.status")}
				</div>
				<div
					role="menu"
					tabindex="0"
					class="dropdown-content z-1 mt-2 w-44 sm:w-52 rounded-box bg-base-100 shadow p-2"
				>
					<ul class="menu menu-sm">
						{#each taskStates as status}
							<li>
								<label
									class="flex items-center gap-2 cursor-pointer text-xs sm:text-sm"
								>
									<input
										type="checkbox"
										class="checkbox checkbox-xs"
										checked={selectedStatuses.includes(status.state)}
										onchange={() => toggleStatus(status.state)}
									>
			...
			<input
				...
				placeholder={$_("tasks.search")}
				class="input input-xs sm:input-sm input-bordered w-full lg:w-auto"
			>
		</div>
	</div>
	...
	<!-- Table -->
	<div
		class="overflow-x-auto rounded-box border border-base-content/5 bg-white-100 h-[300px] sm:h-[400px]"
	>
		<table class="table table-fixed w-full text-xs sm:text-sm">
			...
			<tbody>
				{#each visibleTasks as task}
					<tr
						class="hover transition duration-200 {selectedTasks.includes(task.id) ? 'bg-base-300' : ''}"
					>
						<td>
							<input
								type="checkbox"
						...
						<td class="text-right">
							<div class="dropdown dropdown-left">
								...
								<ul
									tabindex="-1"
									class="dropdown-content menu bg-base-300 rounded-box z-1 w-28 sm:w-30 p-2 shadow-sm text-xs sm:text-sm"
								>
									<li>
										<button
											type="button"
	...
	<!-- Footer -->
	<div
		class="p-2 lg:p-3 border-t text-xs sm:text-sm flex flex-col lg:flex-row justify-between gap-2"
	>
		...
		<div class="flex flex-col lg:flex-row gap-2">
			{#if selectedTasks.length > 0}
				...
				<div class="dropdown dropdown-top dropdown-center">
					...
					<ul
						class="dropdown-content menu p-2 shadow bg-base-300 rounded-box w-36 sm:w-40"
					>
						{#each taskStates as status}
							<li>
								<button
