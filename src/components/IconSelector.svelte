...
<div class="flex flex-col gap-3">
	<div class="form-control">
		<label for="project-logo" class="label">
			<span class="label-text">{$_("projects.icon")}</span>
		</label>
		<input
			id="project-logo"
			type="file"
	...
	{#if previewUrl}
		<div class="alert flex flex-col gap-3">
			<div class="w-full">
				<div class="text-sm font-semibold mb-2">{$_("projects.preview")}</div>
				<div
					class="h-32 w-full rounded bg-base-200 flex items-center justify-center overflow-hidden"
				>
					<img
						src={previewUrl}
						alt="Logo preview"
			...
			{#if needsConfirm}
				<div class="w-full flex flex-col gap-2">
					<div class="text-sm whitespace-normal">
						{$_("projects.iconAffirmation")}
					</div>
					<div class="flex gap-2 flex-wrap">
						<button type="button" class="btn btn-primary" onclick={confirmIcon}>
							{$_("common.accept")}
						</button>
						<button type="button" class="btn btn-ghost" onclick={discardIcon}>
							{$_("common.discard")}
						</button>
					</div>
				</div>
