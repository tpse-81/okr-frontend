...
{:then _isReady}
	<div class="navbar bg-base-100 shadow-sm">
							...
							{:else}
								...
								{#if $userInfoStore?.is_admin}
									...
									<li>
										<a
											href="/user"
											class="text-error flex items-center justify-between"
										>
		...
		{#if $userInfoStore}
			...
			<div class="ml-2"><LanguageSwitch /></div>
			...
			<div class="dropdown dropdown-end">
				...
				>
					<li class="btn btn-ghost" onclick={() => goto("/account")}>
						{$_("nav.account")}
					</li>
...
<style>
.filter-dark {
	filter: invert(78%) sepia(43%) saturate(718%) hue-rotate(335deg)
